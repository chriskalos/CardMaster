...
class CardGameUI(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.tier_decks = []  # Initialize here to ensure it's always available
        ...
        self.show_all_cards_button.clicked.connect(self.show_all_cards)
    ...
    def initUI(self):
        ...
        # Show all cards button for showcase purposes
        self.show_all_cards_button = QPushButton("Show All Cards")
        self.show_all_cards_button.setMinimumSize(160, 60)  # Same size as the Play Card button
        self.show_all_cards_button.setStyleSheet("QPushButton { font-size: 20px; padding: 10px; }")
        self.show_all_cards_button.clicked.connect(self.show_all_cards)
        button_layout.addWidget(self.show_all_cards_button)
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            for i, deck in enumerate(self.tier_decks):
                y_position = y_start + i * (150 + 20)  # Adjust as needed
                self.draw_deck(painter, deck, x_start, y_position)
        finally:
            painter.end()  # Ensure painter is properly ended to avoid resource leaks
    ...
    def show_all_cards(self):
        # Clear all states and decks
        self.animation_states.clear()

        # Assuming get_all_cards fetches all the cards correctly grouped by tiers
        cards = get_all_cards()
        deck1, deck2, deck3, deck4, deck5 = Deck(), Deck(), Deck(), Deck(), Deck()
        decks = [deck1, deck2, deck3, deck4, deck5]

        for card in cards:
            decks[card.tier - 1].cards.append(card)

        self.tier_decks = decks

        # Re-initialize animation states for these new decks
        index = 0
        for deck in self.tier_decks:
            for card in deck.cards:
                self.animation_states[index] = {
                    'width': 100, 'height': 150,
                    'target_width': 100, 'target_height': 150,
                    'small': True,
                    'tooltip_shown': False,
                    'clicked': False,
                    'player_card': True  # assuming these are to be interactable as player cards
                }
                index += 1
    ...
    def draw_card(self, painter, card, x, y, animation_states, index):
        try:
            ...
            print(f"No animation state found for index {index}, skipping")
